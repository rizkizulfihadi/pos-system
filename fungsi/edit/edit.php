...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, merk=?, 
				harga_beli=?, harga_jual=?, satuan_barang=?, stok=?, barcode=?, tgl_update=?  WHERE id_barang=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
    ...
    if (!empty($_GET['jual'])) {
        ...
        if ($hasil['stok'] > $jumlah) {
            ...
            $row1 -> execute($data1);
            echo '<script>window.location="../../index.php?page=kasir#keranjang"</script>';
        } else {
    ...
    if (!empty($_GET['cari_barang'])) {
        ...
        if ($cari == '') {
            ...
            $sql = "select barang.*, kategori.id_kategori, kategori.nama_kategori
					...
					where barang.id_barang like '%$cari%' or barang.nama_barang like '%$cari%' or barang.barcode = '$cari' ";
            $row = $config -> prepare($sql);
            $row -> execute();
            $hasil1= $row -> fetchAll();
            ?>
